...
from graph_memory.validation import validate_twowiki_label_records, validate_twowiki_ranking_records

LOGGER = logging.getLogger("prepare_2wiki")


@dataclass(frozen=True)
class PrepareTwoWikiArgs:
    input: str
    output_input: str
    output_labels: str
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.output_combined is not None:
            write_json(args.output_combined, combined_twowiki_records(task_inputs, task_labels))
        ...
        counts = cast(JsonObject, {
            **prepared.counts,
            ...
            "path_supported_tasks": sum(
                1 for label in task_labels if bool(label.get("gold_dependency_edges"))
            ),
        })
        summary = build_run_summary(
            script="prepare_2wiki.py",
...
def select_valid_raw_examples(raw_records: Sequence[object], *, strict: bool) -> ValidRawExamples:
    ...
    for record_index, raw_record in enumerate(raw_records):
        try:
            parsed_example = parse_twowiki_example(raw_record, record_index=record_index)
            ...
            validate_twowiki_label_records([converted_example.label_record], inputs_by_task_id)
        except ValueError as error:
            if strict:
                raise ValueError(f"Invalid 2Wiki raw example index={record_index}: {error}") from error
...
def prepare_from_raw(args: PrepareTwoWikiArgs) -> PreparedTwoWikiRecords:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Convert labeled 2Wiki examples into leakage-safe ranking and label artifacts.")
    parser.add_argument("--input", required=True, help="Path to labeled 2Wiki raw JSON file.")
    parser.add_argument("--output_input", required=True, help="Path to write 2Wiki ranking record JSON.")
    parser.add_argument("--output_labels", required=True, help="Path to write 2Wiki label record JSON.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fail on the first invalid raw 2Wiki example instead of dropping invalid examples.",
    )
    return parser


def parse_args(argv: Sequence[str] | None = None) -> PrepareTwoWikiArgs:
    namespace = build_parser().parse_args(argv)
    return PrepareTwoWikiArgs(
