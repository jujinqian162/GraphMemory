...
from graph_memory.graphs.config import ProposalGraphBuildConfig
...
LOGGER = logging.getLogger("build_proposal_graphs")


@dataclass(frozen=True)
class BuildProposalGraphsArgs:
    input: str
    output: str
    dataset: DatasetId
    method: str
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = ProposalGraphBuildConfig(
    ...
    outputs = {"proposal_graphs": args.output, "graph_stats": str(stats_path), "run_summary": str(summary_path)}
    ...
    try:
        ...
        LOGGER.info("wrote proposal graphs: %s", args.output)
        LOGGER.info("wrote proposal graph stats: %s", stats_path)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", required=True, help="Path to write proposal graph JSON.")
    parser.add_argument("--max_query_overlap", type=int, default=ProposalGraphBuildConfig.max_query_overlap)
    parser.add_argument("--max_entity_neighbors", type=int, default=ProposalGraphBuildConfig.max_entity_neighbors)
    parser.add_argument("--max_bridge_edges", type=int, default=ProposalGraphBuildConfig.max_bridge_edges)
    parser.add_argument("--use_spacy", action="store_true")
    return parser


def parse_args(argv: Sequence[str] | None = None) -> BuildProposalGraphsArgs:
    namespace = build_parser().parse_args(argv)
    return BuildProposalGraphsArgs(
        input=namespace.input,
        output=namespace.output,
        dataset=cast(DatasetId, namespace.dataset),
        method=str(namespace.method),
