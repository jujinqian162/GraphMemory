...
def main(
    ...
    *,
    ...
    execution = load_stage_execution(
        ...
        description="Train a typed retrieval method from a resolved stage YAML.",
    ...
    with stage_lifecycle(execution.invocation) as observations:
        ...
        write_jsonl(config.metrics, metric_records)
        _ = _write_method_artifacts(config, training_result)
        for key, value in _result_counts(payload, training_result).items():
