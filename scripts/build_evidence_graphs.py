...
from graph_memory.experiment.stage_models import EvidenceGraphStageConfig
...
def main(argv: Sequence[str] | None = None) -> int:
    execution = load_stage_execution(
        ...
        description="Build typed evidence graphs from a resolved stage YAML.",
    ...
    config = GraphBuildConfig(
        max_query_overlap=stage_config.evidence_graph.max_query_overlap,
        max_entity_neighbors=stage_config.evidence_graph.max_entity_neighbors,
        max_bridge_edges=stage_config.evidence_graph.max_bridge_edges,
        use_spacy=stage_config.evidence_graph.use_spacy,
    )
    effective_config = {
    ...
    with stage_lifecycle(execution.invocation) as observations:
        ...
        graph_requests = evidence_graph_build_requests_for_dataset(
        ...
        LOGGER.info("wrote evidence graphs: %s", stage_config.output)
