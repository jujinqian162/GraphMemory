...
def main(
    ...
    *,
    ...
    summary_path = SUMMARY_DIR / "prepare_dataset.summary.yaml"
    effective_config = {
    ...
    inputs = {
        "dataset": args.dataset,
        "sources": {
            dataset_file.split: dataset_file.url for dataset_file in dataset_spec.files
        },
    }
    outputs = {
    ...
    counts = {
        "dataset_files": len(dataset_spec.files),
        "downloaded_files": 0,
        "skipped_files": 0,
    }
    notes: list[str] = []
    ...
    try:
        ...
        for dataset_file in file_iterator:
            ...
            if should_skip(
                destination, dataset_file, verify=not args.no_verify, force=args.force
            ):
...
def _dataset_summary(**values: object) -> dict[str, object]:
    return {
        "version": 1,
        **values,
        "environment": {
...
def should_skip(
    destination: Path, dataset_file: DatasetFile, *, verify: bool, force: bool
) -> bool:
...
def verify_file(path: Path, dataset_file: DatasetFile) -> None:
    if (
        dataset_file.num_bytes is not None
        and path.stat().st_size != dataset_file.num_bytes
    ):
        raise ValueError(
    ...
    if dataset_file.sha256 is not None:
        ...
        if actual != dataset_file.sha256:
            raise ValueError(
                f"Downloaded file checksum mismatch: {path} expected={dataset_file.sha256} actual={actual}"
            )
...
def download_url(url: str, destination: Path, expected_size: int | None) -> None:
    ...
    with urlopen(url) as response:
        total = (
            expected_size
            or int(response.headers.get("Content-Length", "0") or 0)
            or None
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download registered raw datasets into data/<name>/raw."
    )
    parser.add_argument(
        "--dataset",
        required=True,
        help="Dataset registry key to download, e.g. hotpotqa-v1.",
    )
    parser.add_argument(
        "--name",
        required=True,
        help="Dataset directory name under --data_dir, e.g. hotpotqa.",
    )
    parser.add_argument(
        "--data_dir", default="data", help="Root data directory. Defaults to data."
    )
    parser.add_argument(
        "--force",
        action="store_true",
