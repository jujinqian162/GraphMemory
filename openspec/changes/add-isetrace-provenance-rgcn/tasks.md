...
- [x] 1.1 Add failing strict-config tests for `trajectory_source`, `natural_query_source`, and exact `queries.splits.<split>.natural/template` counts; reject retired ratio/test-only/review/target-policy/kind/version fields for ISETrace.
...
- [x] 1.4 Add failing deterministic split tests proving all queries for one trajectory stay together, split assignments depend on `split_seed` rather than training seed, and registered ownership weights produce exact target counts.
- [x] 1.5 Add failing composition tests for exact natural/template counts, template-only train/dev, insufficient-origin failure, train/dev trajectory ownership, and natural-only test.
...
- [x] 2.4 Deterministically enumerate/select template records only from trajectories assigned to train or dev and satisfy exact configured counts without generating test templates.
...
- [x] 6.1 Compute natural and template dev metrics separately; select on natural dev when present and template dev for template-only ablations.
...
- [x] 6.3 Reuse existing ISETrace span Recall/Coverage, Full Support, span F1, MRR, and evidence-density evaluation, adding Coverage@512/1024 alongside Coverage@2048.
...
## 8. Explicit-count and token-budget extension

- [x] 8.1 Replace split/mix ratios with strict per-split natural/template counts and registered trajectory-group ownership weights.
- [x] 8.2 Support natural-zero template-only train/dev preparation and template-dev checkpoint selection.
- [x] 8.3 Add Coverage@512 Tokens and Coverage@1024 Tokens to per-task, aggregate, table, and tracking contracts.
- [x] 8.4 Update maintained docs/specs and rerun broad tests, Ruff, BasedPyright, compileall, diff-check, and strict OpenSpec validation.
