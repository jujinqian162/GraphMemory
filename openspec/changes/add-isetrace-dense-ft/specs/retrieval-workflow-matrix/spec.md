...
#### Scenario: Trainable method lacks lifecycle inputs
- **WHEN** `dense_ft` or `provenance_rgcn` is configured for ISETrace without prepared train, dev, or test inputs
...
- **WHEN** a method retrieves over an ISETrace task whose natural record contains only exact gold spans
- **THEN** the workflow does not derive gold path or edge labels from the input graph
- **AND** path and edge accuracy remain unavailable

### Requirement: Trainable ISETrace methods use method-native views
The ISETrace pair and train stages SHALL select candidate and label projections by the configured trainable method rather than by dataset name alone. Dense-FT SHALL use flat chunks and exact-span-derived flat positives; `provenance_rgcn` SHALL use provenance content candidates, the query-independent graph, and its existing natural/template supervision adapter.

#### Scenario: Build Dense-FT supervision
- **WHEN** the ISETrace method is `dense_ft`
- **THEN** pair and train payloads use flat text requests and flat positive IDs
- **AND** no provenance graph or candidate-neighbor projection is included

#### Scenario: Build provenance R-GCN supervision
- **WHEN** the ISETrace method is `provenance_rgcn`
- **THEN** pair and train payloads retain provenance candidates, provenance graphs, and the existing candidate-neighbor behavior

#### Scenario: Reject a mismatched train view
- **WHEN** an ISETrace train artifact is projected with candidate or label IDs from the wrong method-native view
- **THEN** validation fails before optimization begins
