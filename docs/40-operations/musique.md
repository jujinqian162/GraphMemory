...
The repository supports the answerable MuSiQue split with paragraph-level candidates and decomposition-derived dependency edges. Use the `musique` dataset group; split sources and capacities are declared in `configs/experiment/dataset/musique.yaml`.

```powershell
uv run python -m graph_memory.experiment.plan `
  name=musique_smoke dataset=musique profile=smoke device=cpu `
  'methods=[bm25,bm25_graph_rerank]'

uv run python -m graph_memory.experiment.run `
  name=musique_smoke dataset=musique profile=smoke device=cpu `
  'methods=[bm25,bm25_graph_rerank]'

uv run python -m graph_memory.experiment.status name=musique_smoke
```
