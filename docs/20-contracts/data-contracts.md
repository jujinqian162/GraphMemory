...
The durable natural-query JSONL has four fields: `id`, handle-delimited task `text`, natural-language `query`, and `gold` entries containing `{source, quote}`. Preparation resolves valid records, deterministically selects disjoint train/dev/test trajectory sets from `trajectories.splits`, and includes every authored natural query belonging to a selected natural trajectory. Each selected template trajectory contributes one deterministic template query. Natural gold remains exact `SourceSpan` data compiled from quotes. Template supervision and `query_origin` are sidecar payloads, never request, candidate, graph, or model-feature fields. Test is natural-only.
