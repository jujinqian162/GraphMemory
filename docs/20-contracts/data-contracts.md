...
Preparation content-addresses the natural-query JSONL and pinned trajectory source, whose revision is inferred from dataset registration and checked against authoring metadata when available. It materializes flat chunks, provenance argument/output chunks, immutable graphs, exact labels, query metadata, and deterministic counts. BM25, Dense, and GraphRAG receive flat text requests; `provenance_path` receives the physical graph without training; `provenance_rgcn` receives physical graphs and provenance content candidates through the shared trainable lifecycle. Every test method is evaluated against the derived natural-only exact spans.
