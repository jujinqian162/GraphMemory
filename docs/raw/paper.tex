...
\begin{itemize}
    ...
    query-conditioned diffusion and connected-subgraph retriever plus a
...
learn parameters or inspect dataset identity. Historical dependency-path
and typed-beam strategies are retained only as reproducibility diagnostics.

First, the frozen E5 encoder compares query $q$ with a versioned
schema-owned description of every provenance relation $r$. A
temperature-scaled softmax gives query-conditioned relation affinity
$a_q(r)$. Each stored edge becomes a forward arc and a penalized reverse
arc. Before source normalization, arc strength is
\begin{equation}
\tilde T_q(e)=
\pi(\psi(e))\,a_q(\psi(e))\,c(e)\,\rho(e)\,d(e),
\end{equation}
where $\pi$ is a fixed type prior, $c(e)$ is recorded confidence,
$\rho$ is the reverse-direction factor, and $d$ controls high-degree
hubs. We normalize $\tilde T_q$ independently over each source's outgoing
arcs. Thus synthetic \texttt{feeds} confidence remains a source-local
successor probability; constant real-trace weights are an identity factor.
Invalid field bindings receive no transition probability.

Min--max scaled Dense relevance with a fixed positive numerical floor
defines teleport distribution $y$, preserving the narrow dynamic range of
normalized-encoder cosine scores. Typed Personalized PageRank then diffuses
over candidates and native connector nodes:
\begin{equation}
p^{(t+1)}=(1-\alpha)y+\alpha T_q^{\top}p^{(t)},
\qquad \alpha=0.85,
\end{equation}
with dangling mass returned through $y$ and bounded deterministic
convergence.

Finally, EPGM jointly extracts a compact connected subgraph rather than
selecting each endpoint independently. Candidate prize combines normalized
Dense and PPR relevance minus a fixed inclusion cost. A deterministic budgeted
Steiner-style greedy procedure starts from the strongest prize already in the
Dense top-$k$ incumbent and repeatedly adds the residual minimum-cost connector
path with positive counterfactual marginal. Already-selected tree arcs have
zero residual cost; a candidate outside the incumbent pays both new
$-\log T_q$ connector cost and the prize of the Dense evidence it displaces.
At most $k$ request candidates are selected; tasks, agents, calls, and other
native nodes may act as connectors without consuming the evidence budget.
Accepted membership changes retain Dense-relative order. Stored orientation is
retained in the native trace and selected candidate paths are collapsed to
logical dependencies for shared evaluation. Session-only structural paths remain native-trace evidence
rather than fabricated logical dependencies. If no positive-marginal
multi-candidate connection is selected, the full ranking and scores are exactly
Dense. All relation descriptions and
algorithm parameters are frozen before formal RQ2/RQ3 execution; the new
quality rows remain pending those runs.
...
\textbf{EPGM} denotes the single training-free query-conditioned diffusion
and connected-subgraph retriever from Section~\ref{sec:evidence-tracing}.
Its historical typed-beam and directed dependency-path strategies remain
reproducibility diagnostics only; neither the reported default nor the
diagnostics use task-specific parameter training. The new default's RQ2 and
RQ3 quality rows remain pending formal execution. \textbf{EPGM R-GCN} is its learned
