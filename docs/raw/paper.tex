...
We first evaluate structure-aware evidence retrieval on HotpotQA,
2WikiMultiHopQA, and MuSiQue, comparing lexical and dense retrieval,
heuristic graph reranking, and relation-aware graph encoders. We then
evaluate EPGM on a controlled 2Wiki-derived execution-provenance
benchmark against BM25 and a lightweight non-LLM entity-graph
baseline. We measure evidence recall, complete-support recovery, and,
where graph labels are available, path and edge recall. Relation-aware
encoding consistently improves complete-support recovery, and EPGM
retrieves more complete execution dependencies than the two simulated
trajectory baselines. R-GCN ablations further identify bridge edges
and graph propagation as the main structural contributors.
...
\begin{itemize}
    ...
    \item We introduce a controlled provenance-aware retrieval setting
    and show that execution dependencies improve complete-support
    retrieval over sparse and lightweight non-LLM entity-graph
    baselines.
\end{itemize}
...
\emph{RQ3} asks whether execution-level provenance is more suitable
than lexical retrieval and entity-graph propagation for retrieving
evidence from simulated agent trajectories.
...
For simulated trajectories, \textbf{BM25} provides a sparse baseline.
\textbf{FastGraphRAG-style} is a compact, non-LLM entity-graph
baseline: it deterministically extracts and links entities, combines
lexical and dense seeds with Personalized PageRank, and projects
entity scores back to evidence. It follows the non-LLM entity-search
idea of FastGraphRAG and is not a reproduction of a full GraphRAG
pipeline with LLM-based community summarization or generation.
\textbf{EPGM} performs bounded search over a typed
execution-provenance graph and combines semantic relevance, dependency
completeness, field binding, and grounding into a path score.
...
connectivity rather than path and edge recall for that dataset.
...
\paragraph{Execution-level provenance improves complete dependency
recovery.}
Table~\ref{tab:provenance} compares lexical retrieval, lightweight
entity-graph propagation, and execution-provenance path retrieval.
EPGM outperforms BM25 and FastGraphRAG-style on every listed ranking
metric. Relative to FastGraphRAG-style, EPGM increases Recall@5 from
58.00\% to 63.87\%, Full Support@5 from 19.89\% to 32.83\% (a
65.1\% relative improvement), Full Support@10 from 50.30\% to
57.21\%, and MRR by 6.05 points.
...
\textbf{EPGM} & \textbf{43.21} & \textbf{63.87} & \textbf{78.52} &
\textbf{32.83} & \textbf{57.21} & \textbf{77.91} &
\textbf{28.83} & \textbf{28.73} & 41.10 \\
\hline
\end{tabular}%
}
\caption{Synthetic 2Wiki execution-provenance retrieval results.}
\label{tab:provenance}
\end{table*}

The non-LLM FastGraphRAG-style baseline improves substantially over
BM25 by propagating relevance through entity relations. Its graph,
however, describes textual entity association rather than how a tool
output enters a later call. EPGM instead expands along typed field
bindings and data-flow relations, such as
\texttt{ToolOutput}$\rightarrow$\texttt{ToolCall}$\rightarrow$
\texttt{ToolOutput}. The largest difference appears in Full
Support@5, supporting the interpretation that provenance mainly helps
complete a dependency chain within a small context budget.

EPGM recovers 28.83\% of gold paths and 28.73\% of gold dependency
edges at $k=10$. These absolute values leave substantial room for
learned relation-aware retrieval. Nevertheless, EPGM averages
41.10 ms per query, 61.0\% lower than the 105.53 ms of the
FastGraphRAG-style baseline. BM25 remains fastest, but its Full
Support@5 is only 7.33\%. These latency numbers cover retrieval only;
...
First, the synthetic provenance benchmark is deterministically
derived from question-answering evidence chains rather than collected
from heterogeneous, naturally occurring agent logs. Second, results
use one fixed split and do not yet include multi-seed variance or
bootstrap confidence intervals. Third, the current experiments
evaluate retrieval and path recovery only; they do not directly
measure unsupported-claim reduction or downstream impact tracing.
Finally, the R-GCN ablation concerns the conventional evidence graph,
not the execution-provenance scorer. Direct EPGM ablations should
remove field-binding and provenance-completeness terms and shuffle
data-flow edges; invalidation penalties require a benchmark containing
actual evidence revisions. We therefore interpret the present results
as controlled evidence for execution-provenance retrieval, not as a
final evaluation of the full multi-agent memory lifecycle.

\section{Conclusion}

We proposed Execution-Provenance Graph Memory for evidence tracing in evidence-heavy multi-agent systems. Instead of retrieving isolated memory records, our method represents observations, tool outputs, intermediate claims, verifications, agent decisions, and final answers as a typed provenance graph. This representation enables compact evidence-path retrieval and provides a structural basis for tracing outputs affected by revised evidence. Across three multi-hop evidence-retrieval benchmarks, relation-aware graph encoding consistently improves complete-support recovery and strengthens path retrieval. On a controlled synthetic execution-provenance benchmark, EPGM retrieves more complete evidence dependencies than BM25 and a lightweight non-LLM entity-graph baseline while remaining faster than the graph baseline. Evaluating unsupported-claim reduction and revision impact tracing on naturally occurring agent trajectories remains future work. These results suggest that multi-agent memory should support not only recall, but also provenance, coordination, and auditability.
