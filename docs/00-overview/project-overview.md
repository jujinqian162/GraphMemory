...
The evidence workflow remains runnable. The revision-pinned ISETrace corpus has canonical trajectory adaptation, query-independent provenance graphs, deterministic motif templates, and trajectory-grouped train/dev/test allocation. Natural queries are retained in every split; train/dev may add ratio-derived templates, while test is always natural-only. The current generated natural-query corpus is explicitly unreviewed and is suitable for engineering validation, not final paper claims.
