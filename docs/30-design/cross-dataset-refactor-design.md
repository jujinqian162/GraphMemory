...
状态：设计基线。本文描述下一轮跨数据集架构抽象，不是实施计划，也不表示当前代码已经完成该设计。
...
```text
HotpotQA raw
  -> MemoryTaskInput / MemoryTaskLabels
  -> MemoryGraph
  -> RetrievalMethod ranks memory node ids
  -> evaluation compares gold_evidence_nodes
```
...
- `MemoryTaskInput` 同时承担了 dataset conversion artifact、retriever input、graph builder input 和 evaluation join key 的职责。
- `MemoryItem` 的字段形状偏向 HotpotQA sentence：`source`、`sentence_id`、`position` 和 `document_sentence`。
- BM25、Dense、Memory Stream、Graph Builder、training pair builder 和 evaluator 都直接读取同一套 task shape。
- Evaluation 默认绑定 `gold_evidence_nodes`、graph connectivity 和 ranked node ids，导致 turn full support、session full support、answer quality 等指标没有自然位置。
...
当前 `MemoryTaskInput` 可以被理解为 Evidence Ranking View 的一个 HotpotQA-oriented 版本，而不应继续作为所有未来任务的通用核心语言。
...
这使当前系统可以平滑演进：现有 `MemoryTaskInput` 先被视为 Evidence Ranking View 的当前 artifact 形态，而不是立即推倒重写。
...
- `MemoryTaskInput` 是当前 HotpotQA Evidence Ranking View 的 artifact 形态，不应继续被视为所有 dataset 的永久通用 task type。
- `MemoryTaskLabels` 是当前 Evidence Eval View 的 artifact 形态，不应覆盖 LongMemEval 的 turn/session/answer labels。
