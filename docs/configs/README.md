...
Non-training datasets may configure only a test split. Trainable methods fail fast unless train/dev/test are all available. `dataset/isetrace.yaml` names `trajectory_source`, `natural_query_source`, and explicit `trajectories.splits.<split>.natural/template` counts. These values count trajectories: selected natural trajectories contribute all available authored queries, while selected template trajectories contribute one template query each. The pinned trajectory revision is inferred from dataset registration. ISETrace ignores generic profile split caps, so smoke jobs override these trajectory counts. Review state remains outside the four-field natural-query record and must be frozen operationally before formal runs.
