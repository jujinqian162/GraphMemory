...
这是 learned graph R-GCN trainable retriever 的当前方法配置。它是独立 public method，不是 `dense_rgcn_graph_retriever` 的开关。workflow 会为它构建 method-local proposal graph，然后用 learned edge gate 缩放 R-GCN message edge，并用 rank、edge、sparse 三项 loss 训练。
...
- `encoder`: 冻结 Sentence-Transformers encoder 设置，会写入 checkpoint metadata。
