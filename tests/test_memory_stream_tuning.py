        ...
        "tasks": [
            {
                ...
                "scores": {
                    item["id"]: 10 if item["id"] == "m1" else 1
...
def test_memory_stream_tuning_computes_dense_seed_once_for_all_candidates() -> None:
    ...
    _, candidate_rows = tune_memory_stream(
        ...
        importance_artifact=_importance_artifact(task_inputs),
        grid=grid,
        top_k=2,
...
class _FakeDenseRanker:
    ...
    def rank(self, task_input: MemoryTaskInput) -> list[RankedNode]:
        assert task_input["task_id"] == "hotpot_ex1"
        return [
...
def test_cached_memory_stream_execution_matches_formal_method_ranking() -> None:
    ...
    artifact = _importance_artifact(task_inputs)
