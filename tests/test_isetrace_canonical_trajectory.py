...
def test_isetrace_adapter_preserves_ordered_events_and_pairing() -> None:
    ...
    assert (
        ...
        == adapt_isetrace_record(raw, source_revision=REVISION).fingerprint()
    )


def test_isetrace_adapter_rejects_invalid_arguments_and_orphan_outputs() -> None:
    invalid_arguments = isetrace_record()
    set_call_arguments(invalid_arguments, message_index=2, call_index=0, value="{")
...
def test_streaming_iteration_keeps_compact_rejection_counts(tmp_path: Path) -> None:
    ...
    set_call_arguments(malformed, message_index=2, call_index=0, value="not-json")
    source.write_text(
        "\n".join([json.dumps(isetrace_record()), json.dumps(malformed), "{bad"]),
        encoding="utf-8",
    )
    ...
    assert [trajectory.trajectory_id for trajectory in trajectories] == ["traj_fixture"]
