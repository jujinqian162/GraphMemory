...
def test_public_modules_export_only_stable_entry_points(
    module_name: str, expected: set[str]
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"evaluate_results", "recall_at", "split_metric_tables"},
        ),
        ("graph_memory.graphs", {"GraphBuilder", "GraphIndex", "build_graphs"}),
        (
            "graph_memory.graphs.construction",
            {"EdgeAccumulator", "GraphBuilder", "prepare_graph_input"},
        ),
        ...
        (
            "graph_memory.retrieval.execution",
            {"assemble_ranked_result", "run_retrieval"},
        ),
        (
            "graph_memory.retrieval.methods.flat",
            {"BM25TaskRetriever", "DenseConfig", "ScorePipelineMethod"},
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "scripts.workflow.plan",
        "scripts.workflow.runner",
    ],
)
def test_retired_experiment_compatibility_modules_are_absent(module_name: str) -> None:
    try:
