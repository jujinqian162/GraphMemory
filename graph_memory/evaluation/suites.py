...
def evaluate_evidence(request: EvidenceEvaluationRequest) -> list[MetricRow]:
    aggregate_rows, _ = _evaluate_evidence(request)
    return aggregate_rows

def evaluate_evidence_with_per_task(
    request: EvidenceEvaluationRequest,
) -> tuple[list[MetricRow], list[PerTaskMetricRow]]:
    return _evaluate_evidence(request)
...
def build_evidence_failure_cases(
    request: EvidenceEvaluationRequest,
    *,
    top_k: int = 10,
    limit: int = 0,
