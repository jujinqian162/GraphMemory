...
from graph_memory.graphs.requests import EvidenceGraphBuildNode
...
class BridgeEdgeRule:
    ...
    def add_edges(
        self, graph_input: PreparedGraphInput, accumulator: EdgeAccumulator
    ) -> None:
        ...
        for left, right in combinations(graph_input.request.nodes, 2):
            if _source_group(left) == _source_group(right):
                continue
            shared_entities = (
                graph_input.entities_by_node_id[left.node_id]
                & graph_input.entities_by_node_id[right.node_id]
            )
            cross_title_mentions = _title_mention_score(
                left, right
...
def _title_mention_score(
    left: EvidenceGraphBuildNode, right: EvidenceGraphBuildNode
) -> float:
