...
``dependency_path`` is the reported default. It walks directed dependency edges
from Dense seeds under hard schema gating and folds accepted paths back with
stable insertion, preserving the Dense score multiset. ``ppr_steiner``
(query-conditioned typed diffusion with budgeted connected-subgraph extraction)
and ``typed_beam`` remain reproducibility diagnostics only.
"""
