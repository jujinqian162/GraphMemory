...
def validate_metric_rows(
    rows: object, *, metric_suite: MetricRowValidator | None = None
) -> None:
    ...
    for row in rows:
        ...
        for column in METRIC_COLUMNS:
            ...
            if not math.isfinite(value):
                raise ContractValidationError(
                    f"Invalid metric rows: column={column} must be finite."
                )
            if column == "Retrieval Latency / Query":
                if value < 0.0:
                    raise ContractValidationError(
                        "Invalid metric rows: latency must be non-negative."
                    )
...
def _validate_optional_metric_value(value: object, column: str) -> None:
    ...
    if isinstance(value, bool) or not isinstance(value, int | float):
        raise ContractValidationError(
            f"Invalid metric rows: column={column} must be N/A or in [0.0, 1.0]."
        )
    numeric = float(value)
