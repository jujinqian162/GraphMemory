    ...
    for index, graph in enumerate(graphs):
        if not isinstance(graph, dict):
            raise ContractValidationError(
                f"Invalid graph: record index={index} is not an object."
            )
        ...
        if task_id not in expected_by_task_id:
            raise ContractValidationError(
                f"Invalid graph: task_id={task_id} has no expected graph item ids."
            )
        ...
        if not isinstance(nodes, list):
            raise ContractValidationError(
                f"Invalid graph: task_id={task_id} nodes must be a list."
            )
        if not isinstance(edges, list):
            raise ContractValidationError(
                f"Invalid graph: task_id={task_id} edges must be a list."
            )
...
def expected_item_ids_from_text_requests(
    requests: Sequence[TextRankingRequest],
) -> dict[str, set[str]]:
    return {
        request.task_id: {candidate.item_id for candidate in request.candidates}
        for request in requests
    }


def expected_item_ids_from_graph_requests(
    requests: Sequence[EvidenceGraphBuildRequest],
) -> dict[str, set[str]]:
    return {
        request.task_id: {node.node_id for node in request.nodes}
        for request in requests
    }
    ...
    if isinstance(value, Mapping):
        ...
        for task_id, raw_ids in value.items():
            if not isinstance(task_id, str) or not task_id:
                raise ContractValidationError(
                    "Invalid expected graph item ids: task ids must be non-empty strings."
                )
            expected[task_id] = _string_id_set(
                raw_ids, artifact_name="expected graph item ids", task_id=task_id
            )
        return expected
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
        if all(isinstance(item, EvidenceGraphBuildRequest) for item in value):
            return expected_item_ids_from_graph_requests(
                cast(Sequence[EvidenceGraphBuildRequest], value)
            )
        if all(isinstance(item, TextRankingRequest) for item in value):
            return expected_item_ids_from_text_requests(
                cast(Sequence[TextRankingRequest], value)
            )
    raise ContractValidationError(
        "Invalid expected graph item ids: expected mapping or request sequence."
    )
...
def _string_id_set(value: object, *, artifact_name: str, task_id: str) -> set[str]:
    if not isinstance(value, Iterable) or isinstance(value, (str, bytes, Mapping)):
        raise ContractValidationError(
            f"Invalid {artifact_name}: task_id={task_id} ids must be a sequence."
        )
    ids: set[str] = set()
    for item_id in value:
        if not isinstance(item_id, str) or not item_id:
            raise ContractValidationError(
                f"Invalid {artifact_name}: task_id={task_id} ids must be non-empty strings."
            )
        ids.add(item_id)
    return ids
...
def _validate_graph_nodes(
    ...
    for node in nodes:
        if not isinstance(node, dict):
            raise ContractValidationError(
                f"Invalid graph: task_id={task_id} node is not an object."
            )
        ...
        if node_type not in ALLOWED_NODE_TYPES:
            raise ContractValidationError(
                f"Invalid graph: task_id={task_id} node_id={node_id} unsupported node_type={node_type}."
            )
        if node_id == "q":
            question_count += 1
            _reject_unknown_fields(
                node, QUESTION_NODE_FIELDS, "question graph node", task_id
            )
            if node_type != "question":
                raise ContractValidationError(
                    f"Invalid graph: task_id={task_id} q node must have node_type=question."
                )
            _required_string(node, "text", "question graph node", task_id)
        else:
            _reject_unknown_fields(
                node, GRAPH_ITEM_NODE_FIELDS, "graph item node", task_id
            )
            ...
            if "sequence_index" in node:
                _required_int(
                    node, "sequence_index", "graph item node", task_id, minimum=0
                )
            if "metadata" in node and not isinstance(node["metadata"], dict):
                raise ContractValidationError(
                    f"Invalid graph: task_id={task_id} node_id={node_id} metadata must be an object."
                )
            graph_item_ids.add(node_id)

    if question_count != 1:
        raise ContractValidationError(
            f"Invalid graph: task_id={task_id} must contain exactly one q node."
        )
    return seen_node_ids, graph_item_ids


def _optional_string(
    record: ValidationRecord, field_name: str, artifact_name: str, task_id: str
) -> None:
    if field_name in record and not isinstance(record[field_name], str):
        raise ContractValidationError(
            f"Invalid {artifact_name}: task_id={task_id} field={field_name} must be a string."
        )


def _validate_graph_edge(
    edge: ValidationRecord, graph_node_ids: set[str], task_id: str
) -> None:
    if not isinstance(edge, dict):
        raise ContractValidationError(
            f"Invalid graph: task_id={task_id} edge is not an object."
        )
    ...
    if source not in graph_node_ids:
        raise ContractValidationError(
            f"Invalid graph: task_id={task_id} edge source={source} does not exist in nodes."
        )
    if target not in graph_node_ids:
        raise ContractValidationError(
            f"Invalid graph: task_id={task_id} edge target={target} does not exist in nodes."
        )
    edge_type = _required_string(edge, "edge_type", "graph edge", task_id)
    if edge_type not in ALLOWED_EDGE_TYPES:
        raise ContractValidationError(
            f"Invalid graph: task_id={task_id} unsupported edge_type={edge_type}."
        )
    _required_finite_number(edge, "weight", "graph edge", task_id, minimum=0.0)
    if not isinstance(edge.get("directed"), bool):
        raise ContractValidationError(
            f"Invalid graph: task_id={task_id} edge directed must be boolean."
        )


__all__ = [
    "expected_item_ids_from_graph_requests",
    "expected_item_ids_from_text_requests",
    "validate_graphs",
]
